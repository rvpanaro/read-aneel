...
A Tabela 1 exibe os 11 indicadores selecionados na seção IV (p. 28) da 5ª Edição do Relatório de Indicadores de Sustentabilidade Econômico-Financeira das Distribuidoras. 
...
pander(tab2, caption = "Tabela 1: Indicadores Selecionados. **A1**:Indicadores Base 2017 1T 2017 08 07.xlsx, **A2**: Indicadores Base 2017 2T 2017 11 10.xlsx, **A3**: Indicadores Base 2017 3T 2018 02 20.xlsx, **A4**: Indicadores Base 2018 1T 2018 08 06.xlsx e **A5**: Indicadores Base 2018 1T 2018 08 06.xlsx.")
```
...
Os tipos de dados são um conceito importante para aplicação correta de técnicas estatíticas. As informações ou dados quantitativos (numéricos) são baseados em uma de medição. Em contraste, as informações qualitativas (categóricas) registram qualidades que são descritivas e subjetivas que representam grupos ou características (e.g.: sexo) e podem assumir valores numéricos (e.g.: 1 para mulheres e 0 para homens), observe que não têm significado matemático.

![](img/diag.png)

Os indicadores selecionados são provenientes da combinação de outros atributos de natureza numérica, uma vez que medem distintos aspectos financeiros das distribuidoras. No entanto, além de valores inteiros e reais, encontram-se células com caracteres, o que evidencia outra inconsistência na base de dados.  Em campos de natureza quantitativa, variáveis qualitativas foram consideradas para situações adversas inerentes ao cálculo dos indicadores. No Apêndice B segue a interpretação dada aos valores usados segundo o Glossário da página 27 da 5ª Edição do Relatório.

```{r, echo = F, warnings = F, message = F }
source("read_aneel3.R"); library(pander)
  
dat <- read_aneel3(folder_path = folder_path, sheet = sheet, col_names = header, clean_names = F )
  
  cat <- c("Cxa Líq", "Flx Neg", "Ebitda Neg", "nd")
  
# out_char <-  function(var, cat){
#   
#  i <- which(colnames(dat)== var) 
#  
#  pander( addmargins(table( as.factor(subset(dat[,i], dat[,i] == cat) ) ) ), caption = var)
# }  
# 
# out_char("DLR / EBITDA - QRR", cat)
# 
# for (aux in header){
#   out_char(aux, cat)
# }
